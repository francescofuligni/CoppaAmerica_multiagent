#!/usr/bin/env python3
"""Entry point separato per usare i nuovi file dynamic senza toccare il flusso corrente."""

import os
import argparse
from stable_baselines3 import PPO

from train_ppo_dynamic import train_model
from evaluate_ppo_dynamic import create_video, create_multi_video
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Gestore Addestramento Barca a Vela Multi-Agent (Dynamic Files)"
    )
    parser.add_argument("--train", action="store_true", help="Forza l'avvio del training")
    parser.add_argument(
        "--test-multi",
        action="store_true",
        help="Genera 3 video con seed differenti",
    )
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        default="models/sailing_ppo_dynamic",
        help="Nome file del modello",
    )
    parser.add_argument(
        "--video-file",
        type=str,
        default="videos/sailing_dynamic_demo.mp4",
        help="Nome file video in output",
    )
    parser.add_argument(
        "--ppo-preset",
        type=str,
        default="safe_optimized",
        choices=["legacy", "safe_optimized"],
        help="Preset iperparametri PPO",
    )
    ...
    if args.train or not model_ready:
        print(
            f"Starting DYNAMIC training with {args.n_envs} environments for {args.steps} steps..."
        )
        train_model(
    ...
    if args.test_multi:
        print("\nGenerating multi-agent regatta videos (dynamic files)...")
