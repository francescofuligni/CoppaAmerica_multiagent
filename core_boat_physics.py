...
def compute_vmg_to_target(
    ...
    if target_norm < 1e-6:
        # Se siamo perfettamente sopra il bersaglio, l'avvicinamento aggiuntivo
        # è nullo
        return 0.0
    ...
    # Proiezione scalare (Dot product) del vettore velocità sul vettore
    # direzionale
...
def compute_polar_speed(
    ...
    # nel layer termodinamico di questo ambiente. Normalizziamo in gradi [0,
    # 180].
    angle_deg = normalize_twa_deg(apparent_wind_angle)

    if is_foiling:
        # Foiling (AC75-like): Impossibile stringere il vento puro, si vola
        # solo dai 45-50° in sù
        if angle_deg < 45:
            speed_ratio = 0.0
        elif angle_deg < 55:
            ...
            # Raggiungono un picco ma calano dopo i 140
            speed_ratio = 4.2 - (angle_deg - 140) * 0.03
        else:
            # Stalla se va completamente in poppa filata per l'ombra
            speed_ratio = 3.3 - (angle_deg - 170) * 0.25
    else:
        # Displacement: rotta in acqua, l'andatura di poppa funziona ma è molto
        # più lenta
        if angle_deg < 35:
            speed_ratio = 0.0
        elif angle_deg < 50:
            ...
            # A 180 gradi viaggia tranquilla (non inziale ombra)
            speed_ratio = 1.35 - (angle_deg - 140) * 0.005
