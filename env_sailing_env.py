...
class ImprovedSailingEnv(ParallelEnv):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        # Altissima sui foil (scivola nell'aria limitando l'attrito), bassa in acqua piena.
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> tuple[dict, dict]:
        ...
        for i, agent in enumerate(self.possible_agents):
            # Spawn imbarcazioni all'interno dei Boundaries (x_min, x_max), distanziati verticalmente
        ...
        for agent in self.possible_agents:
            ...
            local_wind_dir, _ = self.wind_field.get_local_wind(self.state[agent]['x'], self.state[agent]['y'])
    ...
    def step(self, actions: dict) -> tuple[dict, dict, dict, dict, dict]:
        ...
        for agent, action in actions.items():
            ...
            self.state[agent]['heading'] = self._normalize_angle(self.state[agent]['heading'] + turn_rate * self.dt)
            ...
            trim_delta = float(np.clip(
                ...
                -self.max_trim_delta_per_step, self.max_trim_delta_per_step,
            ))
            self.state[agent]['sail_trim'] = float(np.clip(prev_trim + trim_delta, 0.0, 1.0))

            # Nota Tecnica: Frenata per virata brusca modificata (più clemente in foiling per attrito inferiore)
            ...
            vmg = compute_vmg_to_target(
                boat_x=self.state[agent]['x'], boat_y=self.state[agent]['y'],
                heading=self.state[agent]['heading'], speed=self.state[agent]['speed'],
                target_x=self.target[agent][0], target_y=self.target[agent][1]
            )
            ...
            # Movimento spaziale
            displacement = self.state[agent]['speed'] * 0.514 * self.dt # nts -> m/s proxy ratio
            ...
            if self.state[agent]['current_leg'] == 1:
                ...
                if by >= self.top_gate_y and gate_left <= bx <= gate_right:
                    ...
                    self.previous_distance[agent] = float(np.linalg.norm(np.array([bx,by]) - self.target[agent]))
                    self.best_distance[agent] = self.previous_distance[agent]
