    ...
    def render_frame(self) -> np.ndarray:
        ...
        for agent in self.env.possible_agents:
            ...
            if agent in self.env.state:
                ...
                rot = np.array(
                    [[np.cos(hdg), -np.sin(hdg)], [np.sin(hdg), np.cos(hdg)]]
                )
                ...
                foil_side = (
                    "Port"
                    if self.env.state[agent].get("active_foil", 1.0) == 1.0
                    else "Stbd"
                )
                foil_str = (
                    f"FOILING ({foil_side})" if is_foiling else f"HULL ({foil_side})"
                )

                dist = np.linalg.norm(np.array([bx, by]) - self.env.target[agent])
                steps = self.env.state[agent].get(
                    "steps_to_target", self.env.step_count
                )
                ...
                trim_percent = int(
                    self.env.state[agent].get("sail_trim", self.env.default_trim_level)
                    * 100
                )
        ...
        if ref_agent in self.env.state:
            ...
            dist_to_gate = abs(
                self.env.target[ref_agent][1] - self.env.state[ref_agent]["y"]
            )

            winner_text = ""
            finished_agents = {
                a: self.env.state[a]["steps_to_target"]
                for a in self.env.possible_agents
                if "steps_to_target" in self.env.state[a]
            }
            if finished_agents:
            ...
            inset_ax.set_xticklabels(
                ["N", "NE", "E", "SE", "S", "SW", "W", "NW"], fontsize=6
            )
            ...
            inset_ax.annotate(
                "",
                xy=(compass_local, 0.65),
                xytext=(0, 0),
                arrowprops=dict(
                    arrowstyle="->", color="darkorange", lw=1.4, linestyle="dashed"
                ),
            )

        self.fig.canvas.draw()

        # Converte il buffer grafico in un array numpy RGB per esportazione
        # video (esito compatibile cross-platform)
        try:
            image = np.array(self.fig.canvas.buffer_rgba(), copy=True)[:, :, :3]
        except AttributeError:
            image = np.frombuffer(self.fig.canvas.tostring_rgb(), dtype=np.uint8)
            image = np.array(
                image.reshape(self.fig.canvas.get_width_height()[::-1] + (3,)),
                copy=True,
            )
